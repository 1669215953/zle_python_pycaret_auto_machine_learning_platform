...
from utils.DataManager import Params,Planet
...
class AutoMLPipeline:
    ...
    def run_predict_model(self):
        if self.history.get_path(Planet.test):
            predictions = predict_model(self.model,data = self.history.get_path(Planet.test))
...
def get_predict(params):
    ...
    history["predictions"]=predictions.to_dict(orient='records')
    DBmanager.add_or_update_history_json(**params.get_uerId_and_missionId(),train_data=params.turn_history())
